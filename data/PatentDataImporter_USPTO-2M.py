...
filepaths = glob('/home/dmlab/Dropbox/DATA/BenchmarkDataset/USPTO-2M/2015_USPTO.json')
...
for filepath in tqdm(filepaths, desc='file processing'):
    ...
    for sentences in data['Abstract'].values:
        ...
        cnt += 1
    for idx, multi_labels in enumerate(data['Subclass_labels'].values):
        if idx <= round(len(data)*(1-test_ratio))-1: train_or_test = 'train'
